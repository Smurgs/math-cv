...
def plot_with_labels(low_dim_embs, labels, filename='target/tsne.png'):
...
def main(args):
    ...
    with tf.Session(graph=graph) as session:
        ...
        average_loss = 0
        for step in range(len(transformed_labels)):
            ...
            if step % 10000 == 0:
                ...
                logging.info('Average loss at step ' + str(step) + ': ' + str(average_loss))
                average_loss = 0

        final_embeddings = normalized_embeddings.eval()
        np.save('target/vector_embeddings.npy', final_embeddings)
